...
/**
 * Ganz simples Programm von mir, welches zwei gleiche Bilder auf Gleichheit überprüft
 * @author Larry Liu
 *
 */
...
public class TestClass {
   ...
   public static void compare (String imagePfadA, String imagePfadB){ //vergleicht zwei Arrays auf Gleichheit
       ...
       try {
    	   ...
    	   pixelOfA = new int[imageA.getWidth() * imageA.getHeight()]; //Array, welches so groß ist wie die Anzahl der Pixel
	       pixelOfB = new int[imageB.getWidth() * imageB.getHeight()];
	    	   ...
	    	   for (int i=0; i<imageA.getHeight();i++){
	    		   ...
	    		   for (int z=0; z<RGBarray2.length;z++){ //Füllt das Array mit RGB Werten von Bild B
